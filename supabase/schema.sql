-- Phase 2 identity + Phase 3 wallet. Paste into the Supabase SQL editor (or `supabase db push`).
-- Cards stay on the CDN; this database stores who is playing and the token ledger.
-- Existing projects that already ran Phase 2 should paste supabase/phase3.sql instead.
-- Use the transaction pooler URL in DATABASE / service client from day one.
...
create table if not exists players (
  ...
  trivia_answered integer not null default 0,
...
create table if not exists claims (
  ...
  battler_id text not null,
  claimed_at timestamptz not null default now(),
  unique (player_id)
);

-- Floor-safe: one live claim per LinkedIn card. Guest ids are `guest:<player uuid>` so they never collide.
create unique index if not exists claims_battler_unique on claims (battler_id);

alter table players enable row level security;
alter table claims enable row level security;

-- Browser never talks to these tables. Next.js API routes use the service role.
drop policy if exists "no client access players" on players;
drop policy if exists "no client access claims" on claims;

create or replace function claim_battler(p_player uuid, p_battler text)
returns void
language plpgsql
as $$
begin
  if exists (select 1 from claims where battler_id = p_battler and player_id <> p_player) then
    raise exception 'taken' using errcode = 'P0001';
  end if;
  delete from claims where player_id = p_player;
  insert into claims (player_id, battler_id) values (p_player, p_battler);
  update players
     set is_guest = false,
...
create or replace function claim_guest(p_player uuid, p_name text)
returns text
language plpgsql
as $$
declare
  gid text;
begin
  gid := 'guest:' || p_player::text;
  delete from claims where player_id = p_player;
  insert into claims (player_id, battler_id) values (p_player, gid);
  update players
     set is_guest = true,
         display_name = p_name
   where id = p_player;
...
begin
  ...
  -- Keep the surviving claim. If keep has none, take drop's.
...
-- ----- Phase 3 (same as supabase/phase3.sql) -----
